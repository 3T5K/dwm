...
tile(Monitor *m)
{
    unsigned int i, n, h, mw, my, ty, si, ma, sa, ns;
    Client *c, *xr[2];
    ...
    for (i = ma = sa = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < m->nmaster) {
            xraycfg( c
                   , m->stacks.master ? XrayGrpTileMaster : XrayGrpNone
            ...
            resize( c
                  , m->wx + ogap
                  , m->wy + ogap + my * !m->stacks.master
                  , mw - (ns ^ 3) * ogap - 2 * c->bw - (ns - 1) * (igap / 2)
                  , h - 2 * c->bw - igap * !m->stacks.master
                  , False
                  );
            ...
            xraycfg( c
                   , m->stacks.stack ? XrayGrpTileStack : XrayGrpNone
            ...
            resize( c
                  ...
                  , m->wy + ogap + ty * !m->stacks.stack
                  , m->ww - (ns ^ 3) * ogap - mw - 2 * c->bw - (ns - 1) * (igap / 2)
                  , h - 2 * c->bw - igap * !m->stacks.stack
                  , False
                  );
    ...
    if (!ma && m->stacks.master)
        xrayfallback(XrayGrpTileMaster);

    if (!sa && m->stacks.stack)
